...
function AfficherFilm(selectionsfilm) {
  // récup infos Json et boucle affichages
    ...
    $.getJSON(url + selectionsfilm + api_key + "&page=" + suivant, function(data) {
  ...
  // récup infos Json et boucle affichages
    ...
    $.getJSON(urlSerie + selectionsfilm + api_key + "&page=" + suivant, function(data) {
        for (var i = 0; i < data.results.length; i++) {
            ...
            var poster = posterPaths + data.results[i].poster_path;
         //affichage du résultat dans le carousel
                $(".series").append("<div class='carousel-item'><div class='card card-body'><h2>"+ data.results[i].name + "</h2><center><img width='200' class='carousel_index' src='" + poster + "' alt='First slide'></center><a href='movie.php?id=" + data.results[i].id + "&type=tv' class='btn btn-secondary'>voir plus </a></div></div></div>");
                console.log(poster);
